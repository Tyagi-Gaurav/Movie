...
public class MovieServiceImpl implements MovieService {
    ...
    public Mono<ContentData> getMovieBy(String contentId) {
        ...
        return Mono.zip(titleInfo, principalsInfo).map(data -> {
            final Aka aka = data.getT1();
            final Principal principal = data.getT2();

            return new ContentData(aka.titleId(), ContentData.SubContent.from(aka.subAkas()),
                    ContentData.Person.from(principal.subPrincipals()));
        });
    }
}
